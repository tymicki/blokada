...
class BlockaVpnMain {
    ...
    fun enable() = async(context) {
        ...
        set(BlockaVpnState::class.java, BlockaVpnState(true))
    }

    fun disable() = async(context) {
        v("disabling blocka vpn")
        set(BlockaVpnState::class.java, BlockaVpnState(false))
    }
    ...
    fun deleteLease(lease: RestModel.LeaseRequest) = async(context) {
        v("deleting lease")
        leaseManager.deleteLease(accountManager.state, lease.gatewayId)
